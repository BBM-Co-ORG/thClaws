...
- **Live teammate sessions** — open each teammate’s Chat/Terminal from Team, send inbox messages, and stop that teammate while the rest of the team continues. Sidebar navigation keeps active work running. [Usage and architecture](docs/team-sessions.md).
- **Knowledge bases (KMS) + `/dream`** — per-project and per-user wikis under `.thclaws/state/kms/<name>/pages/`, indexed by a one-line `index.md`. Grep + read (no embeddings), following Andrej Karpathy's LLM-wiki pattern. `/dream` mines your recent sessions in the background and writes a dated audit-trail page to review with `git diff`.
...
- **Browse without interrupting** — switch Chat/Terminal to another saved lead session while the current turn continues. [Behavior and verification](docs/lead-session-viewing.md).
